...
def HypeParameterSpace():
    ...
    att_drop = {'name': 'att_drop', 'type': 'choice', 'values': [0.1, 0.2]} #0.1
    ...
    sent_threshold = {'name': 'sent_threshold', 'type': 'choice', 'values': [0.9, 0.925, 0.95]}
    ir_name = {'name': 'score_model_name', 'type': 'choice', 'values': ['MLP']}
    task_name = {'name': 'task_name', 'type': 'choice', 'values': ['doc']} ## doc, doc_sent, doc_sent_ans
    ...
    with_graph = {'name': 'with_graph', 'type': 'choice', 'values': [0, 1]} # 0, 1
...
def generate_random_search_bash(task_num):
    ...
    for i in range(task_num):
        task_id, parameter_id = random_search_job.single_task_trial(i+100)
